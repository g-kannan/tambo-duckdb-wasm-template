...
// JSON Schema interface for elicitation forms
...
function isSingleEntryMode(request: TamboElicitationRequest): boolean {
  const schema = request.requestedSchema as JsonSchema;
  ...
  return (
    fieldSchema.type === "boolean" || (fieldSchema.type === "string" && "enum" in fieldSchema)
  );
}
  ...
  const [formData, setFormData] = useState<Record<string, unknown>>(() => {
    ...
    fields.forEach(([name, fieldSchema]) => {
      if (fieldSchema.default !== undefined) {
        initial[name] = fieldSchema.default;
      }
    });
    return initial;
  });
